package link.smartwall.kygj.questionbank.adapter.error;
 ...
 * 题库科目章节数据Adapter
 */
public class ErrorQuestionAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    private Context context;
    private List<BaseItem> itemList;
    private LayoutInflater mInflater;
    private OnScrollListener mOnScrollListener;

    public ErrorQuestionAdapter(Context context, List<BaseItem> itemList) {
    ...
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case Types.TYPE_SUBJECT:
                ...
                return new ErrorSubjectViewHolder(context, view);
            case Types.TYPE_CHAPTER:
                view = mInflater.inflate(R.layout.recycleview_item_child, parent, false);
                return new ErrorChapterViewHolder(context, view);
            default:
                view = mInflater.inflate(R.layout.recycleview_item_parent, parent, false);
                return new ErrorSubjectViewHolder(context, view);
        }
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        switch (getItemViewType(position)) {
            case Types.TYPE_SUBJECT:
                ErrorSubjectViewHolder parentViewHolder = (ErrorSubjectViewHolder) holder;
                parentViewHolder.bindView((Subject) itemList.get(position), position, itemClickListener);
                break;
            case Types.TYPE_CHAPTER:
                ErrorChapterViewHolder childViewHolder = (ErrorChapterViewHolder) holder;
