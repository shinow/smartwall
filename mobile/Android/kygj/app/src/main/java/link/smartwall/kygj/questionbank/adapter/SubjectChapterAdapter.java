 ...
 * 题库科目章节数据Adapter
 */
public class SubjectChapterAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    private Context context;
    private List<BaseItem> itemList;
    private LayoutInflater mInflater;
    private OnScrollListener mOnScrollListener;

    public SubjectChapterAdapter(Context context, List<BaseItem> itemList) {
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        switch (getItemViewType(position)) {
            case Types.TYPE_SUBJECT:
                SubjectViewHolder parentViewHolder = (SubjectViewHolder) holder;
                parentViewHolder.bindView((Subject) itemList.get(position), position, itemClickListener);
                break;
            case Types.TYPE_CHAPTER:
                ChapterViewHolder childViewHolder = (ChapterViewHolder) holder;
